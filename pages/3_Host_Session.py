...
if not question_sets:
    ...
    options = {
        f"{qs['title']} — {len(qs.get('questions', []))} Qs, {qs.get('time_limit', 15)}s/q": qs
    ...
    with col1:
        st.info(f"⏱ {chosen_qs.get('time_limit', 15)} seconds per question")
        st.info("👥 Up to 5 players can join")
