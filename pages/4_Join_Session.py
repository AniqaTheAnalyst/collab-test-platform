    ...
    if remaining <= 0 and not already_answered:
        ...
        if code:
            api_post("/sessions/answer", {
                ...
                "q_index": q_index, "chosen": "",
                "correct": q["answer"], "pts": 0, "time_taken": time_limit,
            })
        st.rerun()
        ...
        if not already_answered:
            ...
            if st.button("Submit", key=f"sub{q_index}"):
                ...
                st.session_state["answers"].append({
                    ...
                    "got": got, "confidence": confidence,
                    "pts": pts, "explanation": q.get("explanation", "")
                })
                if code:
                    api_post("/sessions/answer", {
                        "code": code, "player_name": player_name,
                        "q_index": q_index, "chosen": ans,
                        "correct": q["answer"], "pts": pts, "time_taken": time_taken,
                    })
                st.rerun()
        else:
